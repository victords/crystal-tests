...
module MiniGL
  ...
  class Sprite
    ...
    #            milliseconds).
    def animate(indices : Array(Int32), interval : Int32)
      @animate_once_control = 0 if @animate_once_control != 0

      @anim_counter += 1
      if @anim_counter >= interval
        @index_index += 1
        @index_index = 0 if @index_index >= indices.size
    ...
    def animate_once(indices : Array(Int32), interval : Int32)
      return if animate_once_internal(indices, interval)
    ...
    def animate_once(indices : Array(Int32), interval : Int32, &block)
      return if animate_once_internal(indices, interval)
    ...
    private def animate_once_internal(indices : Array(Int32), interval : Int32)
      if @animate_once_control == 2
        return true if indices == @animate_once_indices && interval == @animate_once_interval
        @animate_once_control = 0
      end
      ...
      @anim_counter += 1
      return true unless @anim_counter >= interval
      false
    end
  ...
  class GameObject < Sprite
    ...
    def initialize(x, y, w, h, img, img_gap = Vector.zero, sprite_cols = nil, sprite_rows = nil, mass = 1_f32, retro = nil)
      @x = x; @y = y
      @w = w; @h = h
      # @img_gap = img_gap
      @mass = mass
    ...
    #
    # *Obs.:* This method accepts named parameters.
    # def draw(map = nil, scale_x = 1, scale_y = 1, alpha = 0xff_u32, color = 0xffffff_u32, angle = nil, flip = nil, z_index = 0, round = false)
    #   if map.is_a? Hash
    #     scale_x = map.fetch(:scale_x, 1)
    #     scale_y = map.fetch(:scale_y, 1)
    #     alpha = map.fetch(:alpha, 0xff)
    ...
    #     @img[@img_index].draw (round ? x.to_i : x.to_f), (round ? y.to_i : y.to_f),
    #                           z_index, (flip == :horiz ? -scale_x : scale_x),
    #                           (flip == :vert ? -scale_y : scale_y), color
    #   end
    # end
